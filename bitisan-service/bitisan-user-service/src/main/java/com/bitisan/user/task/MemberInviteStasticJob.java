...
import com.bitisan.user.entity.MemberInviteStasticRank;
...
import com.bitisan.user.vo.MemberPromotionStasticVO;
...
//    private JavaMailSender javaMailSender;
    ...
    /**
     * 每日2点处理，统计用户推广币币手续费返佣结果(0 0 1 * * *)，总榜
     */
    ...
    public void stasticMemberInviteAll() {
    	...
    	while(true) {
	        ...
	        if (all != null && all.size() > 0) {
	        	for(Member item : all) {
	        		if(item.getId() > 10000) {
	        			List<MemberTransaction> transactions = memberTransactionService.queryAllByMember(item.getId(), TransactionType.PROMOTION_AWARD);
