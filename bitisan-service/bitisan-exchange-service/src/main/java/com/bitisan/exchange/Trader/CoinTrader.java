package com.bitisan.exchange.Trader;

import com.alibaba.fastjson.JSON;
import com.bitisan.constant.ContractOrderDirection;
...
import com.bitisan.exchange.entity.MergeOrder;
import com.bitisan.exchange.entity.TradePlate;
        ...
        if(trades.size() > 0) {
            ...
            if(trades.size() > maxSize) {
                ...
                for(int index = 0;index < size;index += maxSize){
                    ...
                    rocketMQTemplate.convertAndSend("exchange-trade",JSON.toJSONString(subTrades));
                }
            }
            else {
                rocketMQTemplate.convertAndSend("exchange-trade",JSON.toJSONString(trades));
            }
        }
    }
        ...
        if(orders.size() > 0) {
            ...
            if(orders.size() > maxSize){
                ...
                for(int index = 0;index < size;index += maxSize){
                    ...
                    rocketMQTemplate.convertAndSend("exchange-order-completed", JSON.toJSONString(subOrders));
                }
            }
            else {
                rocketMQTemplate.convertAndSend("exchange-order-completed", JSON.toJSONString(orders));
            }
        }
    }
